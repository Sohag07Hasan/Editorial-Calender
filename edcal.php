...
class EdCal {
	...
	function create_reminder(){		
		if($_REQUEST['reminderset'] == 'yes' && isset($_REQUEST['date'])) :
			...
			elseif($time == '12:30 AM'){
				$time = '00:30:00';
			}
			else{
				if(preg_match('/AM/',$time)){
				$time = preg_replace('/AM/','',$time);
				$time = trim($time) . ':00';
				}
			}
	        ...
	        if( $this->supports_custom_types ) {
		        ...
		        foreach ($post_types as $post_type) {
                    ...
                    if ($show_this_post_type) {
						if(current_user_can('calendermenu') && $post_type == 'reminderagent') :
							$page = add_submenu_page('edit.php?post_type=' . $post_type, __('Calendar', 'editorial-calendar'), __('Calendar', 'editorial-calendar'), 'calendermenu', 'cal_' . $post_type, array(&$this, 'edcal_list_admin'));
                        else:
    ...
    function edcal_list_admin() {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function(){
                ...
                edcal.str_addPostLink = <?php echo($this->edcal_json_encode(__('New Reminder', 'editorial-calendar'))) ?>;
